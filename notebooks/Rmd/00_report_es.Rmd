...
jupyter_core        5.7.2
-----
Python 3.12.4 | packaged by conda-forge | (main, Jun 17 2024, 10:04:44) [MSC v.1940 64 bit (AMD64)] Windows-10-10.0.19045-SP0
```
...
Las siguientes comparaciones serán en torno a cuantas campañas han aceptado los
...
Con el objetivo de predecir el comportamiento de los clientes para poder así
seleccionar aquellos clientes que tengan más probabilidades de adquirir una
oferta (aceptar próximas campañas) se han utilizado varios modelos de
predicción. Los modelos utilizados son:
...
La variable a predecir (y) es la correspondiente a la sexta campaña 'Response',
variable dicotómica siendo 0 la negativa a la campaña y 1 la aceptación de la
misma.

Al realizar la regresión lineal múltiple por pasos como mejores variables
predictoras (X) se obtuvieron las 12 siguientes: 'MntWines',
'NumStorePurchases', 'Z_CostContact', 'Z_Revenue', 'ChildrenHome', 'Recency',
'Income_cat_enc', 'Year_Old', 'NumCatalogPurchases', 'MntFishProducts',
'Education_enc' y 'AcceptedCmpTotal'. Son las variables que se utilizarán para
todos los modelos, probando con el resto de variables que se han considerado
importantes como predictoras en ocasiones pero se ha observado en las pruebas
que no aporta a la predicción.

Teniendo en cuenta el tamaño de la muestra e intentando evitar todo el
solapamiento posible, se ha ido variando el los parámetros que componen cada
modelo para encontrar el tamaño de la muestra de entrenamiento y prueba
adecuados con los mejores resultados. En la siguiente tabla se pueden encontrar
parte de los resultados para cada modelo:
...
table <- data.frame(
  Model = c("Sw. Reg.", "D. Trees", "R. Forest", "G. Boosting", "kNN", "SVM", "NN"),
  Accuracy = c("", "", 0.90, 0.884, 0.821, 0.87, 0.809),
  Precision = c("", "", 0.773, 0.737, 0.343, 0.682, 0.700),
  Recall = c("", "", 0.31, 0.389, 0.167, 0.224, 0.329),
  `F1-Score` = c("", "", 0.44, 0.509, 0.224, 0.337, 0.448),
  MSE = c("", 0.105, 0.098, 0.116, 0.179, 0.134, 0.116),
  MAE = c("", 0.199, 0.098, 0.116, 0.179, 0.134, 0.128),
  RMSE = c("", 0.209, 0.312, 0.341, 0.423, 0.366, 0.357),
  `R^2^` = c(0.304, "", 0.902, 0.884, 0.821, 0.866, 0.053),
  AUC = c("", "", 0.85, 0.83, "", 0.83, 0.844)
)

table <- kable(table, row.names = FALSE) %>%
  kable_styling(full_width = FALSE) %>%
  row_spec(0, bold = TRUE, color = "#ffffff", background = "#232629") %>%
  kable_styling(latex_options = "striped", 
                position = "center", 
...
Teniendo en cuenta los valores anteriores, destacar el *Accuracy* de todos los
modelos en general que superan el 0,8 hasta 0.9, siendo los valores más
elevados y cercanos a 1 mejor. Por otra parte, también se ha obtenido un
coeficiente R^2^ elevado en Random Forest, Gradient Boosting, kNN y SVM,
indicando como el modelo se ajusta a los datos, siendo la variabilidad
explicada de estos.

Además, se tendrá en cuenta la importancia de las características para las
predicciones, esto refleja qué variables de los clientes serían las más
importantes según los análisis para la decisión de aceptar o no la siguiente
campaña. La siguiente tabla lista las características más importantes y su
nivel de importancia:

```{r}
table <- data.frame(
  Característica = c("AcceptedCmpTotal", "Recency", "MntWines", "NumCatalogPurchases", "NumStorePurchases", "Year_Old"),
  `D. Trees` = c(0.656903, 0.237129, 0.058948, 0.047021, NA, NA),
  `R. Forest` = c(0.263579, 0.170859, 0.152675, 0.081172, 0.072055, 0.071732),
  `G. Boosting` = c(0.351229, 0.190092, 0.103166, 0.082730, 0.085088, 0.055492),
  SVM = c(0.6952, -0.0099, 0.0002, 0.0421, -0.0699, -0.0032)
)
...
Se puede observar como de forma consistente la característica con más
importancia para tener en cuenta la predicción es 'AcceptedCmpTotal' o el
número total de campañas aceptadas por el cliente. Le sigue 'Recency' o tiempo
que ha transcurrido desde la ultima compra, 'MntWines' o dinero gastado en
vinos, y 'NumCatalogPurchases' como número de compras a través del catálogo. La
importancia de estas tres últimas sigue el mismo orden salvo por las compras a
través del catálogo que en Gradient Boosting le supera las compras a través de
la tienda, pero sólo por 0.002. Así mismo, los valores de importancia son muy
bajos salvo por el número total de campañas aceptadas que oscila ente 0.26 y
0.69.

Antes de entrar en detalle con los modelos más relevantes, todos los análisis
están desarrollados en el Notebook de jupyter 04_predictive.ipynb.

## Modelos destacados

### Random Forest

Random Forest con la muestra empleada es el modelo con mayor *accuracy* y
coeficiente de determinación R^2^ para la predicción de 'Response'. Su matriz
de confusión muestra que de las predicciones el 0.3 son Verdaderas Positivas y
el 0.01 Falsas Positivas, mientras que las Verdaderas Negativas son 0.98 y
Falsas negativas 0.69. Relativo a lo mencionado, el Área bajo la curva (AUC) de
la curva ROC es de 0.85, mostrando como de probable es que el modelo haga
correcta la clasificación.

Importancia de las características según Random Forest:
![](images/clipboard-2435151577.png)

En la muestra de prueba total de 441 clientes para Random Forest, 55 de ellos
fueron los que aceptaron la próxima campaña. Se obtiene un *recall* de 0.31 y
*precision* de 0.77, siendo la habilidad para detectar los positivos reales y
la proporción de predicciones correctas en general respectivamente.

Arbol de decisión para Random Forest: ![](images/clipboard-1976038787.png)

### Gradient Boosting

Con Gradient Boosting en la precisión se obtienen menores valores para
*accuracy* y R2 (0.88; 0.88), pero así mismo la predicción parece ser más
precisa obteniendo un valor F1-score de 0.50 en comparación de 0.44 del Random
forest, mostrando que *recall* y *precision* son más equilibradas. El *recall*
en este caso es superior, de 0.40 mientras de disminuye la *precision* a 0.69.
Se refleja en la matriz de confusión que los Verdaderos Positivos y Falsos
Positivos son 0.40 y 0.038 respectivamente, mientras que los Verdaderos
Negativos y Falsos Negativos son 0.96 y 0.59.

Importancia de las características según Gradient Boosting (XGBoost):
![](images/clipboard-452312388.png)

Importancia de las características según Gradient Boosting
(GradientBoostingClassifier): ![](images/clipboard-3667380716.png)

Arbol de decisiones para Gradient Boosting: ![](images/clipboard-209707449.png)

# Conclusiones

En las relaciones, se observa de manera general que aquellas personas que
tienen mayores ingresos son las que más gastan. También se puede ver como los
mayores gastos de los clientes son en vino y productos cárnicos, en general
comprando a través del catálogo. Si nos centramos en aquellas personas que han
aceptado alguna campaña (Grupo_Y en adelante), cuantos más niños tienen en casa
menos consumen con la excepción de las ofertas cuya relación es mayor que del
grupo de personas que no aceptaron campañas (Grupo_N) (r=0.65; r=0.38).
...
Sería posible llamar la atención de las personas que no suelen aceptar campañas
si ésta se realiza con orientación a los hijos o productos relacionados.

Teniendo en cuenta los modelos para predecir el comportamiento del consumidor y
así seleccionar los clientes con mayor probabilidad de adquirir la oferta para
la próxima campaña, se puede observar que los resultados no son muy
alentadores. Aunque el valor *accuracy* para los modelos es elevado, en el
momento de valorar la matriz de confusión y otras métricas como *precision* o
*recall*, se puede observar que no tienen un gran porcentaje de aciertos,
aunque los que tengan es poco probable que sean falsos positivos.

Por otra parte, tanto para Random Forest como Gradient Boosting, gracias a la
matriz de confusión que refleja la probabilidad de verdaderos y falsos
positivos, se puede observar que en ambos modelos aunque la probabilidad de
hallar un positivo es baja (0.3 o 0.4 respectivamente), cuando se consigue es
muy probable que sea un verdadero positivo ya que los falsos positivos tienen
una probabilidad escasa (0.01 y 0.03 respectivamente). Lo opuesto pasa cuando
el resultado es negativo, que aunque sea muy probable hallar un verdadero
negativo, los falsos negativos superan al azar.

Si a la hora de elegir los clientes más afines a aceptar la campaña,
hipotéticamente hablando, conseguimos localizar ese 0.3 de verdaderos
positivos, ya sería una mejora respecto a los resultados de la muestra en la
campaña piloto que tuvo un éxito del 15%. Para ello se obtuvieron los arboles
de decisiones de ambos modelos.

Para ejemplificar la toma de decisiones de los clientes potenciales, teniendo
en cuenta el árbol de decisión de Random Forest, los clientes más afines a
aceptar la próxima campaña serían aquellos que tengan en 'Recency' un valor
entre 24.5-94.5, de estos que en 'MntWines' hayan gastado menos de 895 y luego
aceptaran menos de 3 campañas o en 'NumCatalogPurchases' tenga preferiblemente
un valor superior a 3.5. Se podría filtrar por con estas especificaciones en
las variables dadas para obtener una muestra de clientes a quienes contactar.

Por último, como limitación principal mencionar el tamaño de la muestra (2240)
sobre todo para la realización de predicciones, ya que, como se mencionaba
anteriormente, sólo una pequeña parte de los clientes aceptaron alguna campaña
en esta muestra piloto. A futuro, si se implementa una nueva campaña con su
respectivo registro de clientes, se puede usar la muestra para seguir
entrenando los modelos y conseguir una predicción más segura.
